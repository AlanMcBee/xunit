	...
	Task Execute(string operationID)
	{
		return Task.Run(async () =>
		{
			...
			try
			{
				...
				{
					CancelThunk = () => !cancellationRequested.Contains(operationID),
					DiagnosticMessageSink = this,
					FailSkips = projectAssembly.Configuration.FailSkipsOrDefault,
					FailWarn = projectAssembly.Configuration.FailWarnsOrDefault,
					LongRunningTestTime = TimeSpan.FromSeconds(projectAssembly.Configuration.LongRunningTestSecondsOrDefault),
				};

				using var resultsSink = new ExecutionSink(projectAssembly, discoveryOptions, executionOptions, AppDomainOption.NotAvailable, shadowCopy: false, callbackSink, sinkOptions);
